      ...
      <ul className="roster-whitelist">
        {
          user_roster.map( (user_contact, index) => {
              return (
                <Link to={`/conversation/${user_contact.jid}?source=contacts`} key={index}>
                  <li className="roster-item"  jid={user_contact.jid}>
                    ...
                    <div className="roster-item-name">
                        <div className="roster-item-name-text">
                            <span className="roster-item-name-text-name">{user_contact.name}</span>
                            <span className="roster-item-name-text-status">{user_contact.status}</span>
                        </div>

                        <div style={{paddingRight: "10px"}}>
                          <svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" fill="#008069" className="bi bi-telephone-fill" viewBox="0 0 16 16">
