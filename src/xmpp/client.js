...
import { newDirectMessage, updateLastMessage } from '../features/messages/messageSlice';
...
class XmppClient{
    ...
    static isReady(){
        if(this.xmpp.socket && this.xmpp.socket.socket.readyState){
            console.log('Socker ReadyState: ', this.xmpp.socket.socket.readyState);
        }
    ...
    static async getClientAvatar(){
        let iq_get_vcard = new xml.Element('iq', {from: 'peter@localhost', type: 'get', id:'get-client-avatar'});
	    iq_get_vcard.c('vCard', {xmlns: "vcard-temp"});
        await this.send(iq_get_vcard);
    }

    static async joinGroup(roomname = 'friends'){
        let iq_join_muc = new xml.Element('presence', {from: 'samuel@localhost', to:`${roomname}@conference.localhost/samharris`, id:'join-muc'});
	    iq_join_muc.c('x', {xmlns: "http://jabber.org/protocol/muc"});
        await this.send(iq_join_muc);
    }

    static async sendMessage(to, body){
        let message = xml("message", { type: "chat", to: to, id: this.generateId(5)}, xml("body", {}, body));
    ...
    static createConfigurableRoom(){
        ...
        iq_configurable_room.c('query', {xmlns: 'http://jabber.org/protocol/muc#owner'})
    }

    static checkAndResolveMessageAck(newmsg){
        // REFER: https://xmpp.org/extensions/xep-0184.html
        newmsg.children.forEach(async child => {
            if(child.attrs.xmlns && child.attrs.xmlns.includes("urn:xmpp:receipts")){
                let message_ack = new xml.Element('message', {id: 'message-ack', to: newmsg.attrs.from.split('/')[0]});
                message_ack.c('received', {xmlns: 'urn:xmpp:receipts', id: newmsg.attrs.id});
    ...
    static async processStanza(stanza){
        ...
        if(stanza.is('presence')){
            if(stanza.attrs.type === 'available'){
                //store.dispatch(updateOnlineUsers());
                console.log(`%c Available Presence: ${stanza}`, "color: green; font-size: 0.85rem; font-family: ubuntu; font-style: italic");
            }else if(stanza.attrs.type === 'unavailable'){
                //store.dispatch(updateOnlineUsers());
                console.log(`%c Unavailable Presence: ${stanza}`, "color: red; font-size: 0.85rem; font-family: ubuntu; font-style: italic");
            }
        }
            ...
            if(stanza.is("presence")){
                if(stanza.attrs.id === 'create-instant-muc'){
                    ...
                    stanza.children.forEach(child =>{
                        ...
                        if( namespace && namespace.includes('http://jabber.org/protocol/muc#user')){
                    ...
                    if(status_codes.some(status => status === '110') && status_codes.some(status => status === '201')){
                        let from = stanza.attrs.from;
                        let iq_confirm_instant_room = new xml.Element('iq', {to: from, id: 'confirm-instant-room', type: 'set'});
        		            iq_confirm_instant_room.c('query', {xmlns: 'http://jabber.org/protocol/muc#owner'});
                        	iq_confirm_instant_room.c('x', {xmlns: 'jabber:x:data', type: 'submit'});
        ...
        if(stanza.is("message")){
            ...
            else if(stanza.attrs.id === 'message-ack' && stanza.children.some(child => child.name === 'received')){
                //update redux state and subsequently the view to signify the delivery of the message
                stanza.children.forEach(child => {
                    if(child.attrs)
                        console.log(`Your message with id ${child.attrs.id} has been received by the receipient`);
                })
            }
